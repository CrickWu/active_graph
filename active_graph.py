...
class Net(torch.nn.Module):
    def __init__(self, args, dataset):
        ...
        self.conv1 = GCNConv(dataset.num_features, args.hid_dim)
        self.conv2 = GCNConv(args.hid_dim, dataset.num_classes)
...
def active_learn(k, data, model, optimizer, args):
    print('Using', device, 'for neural network training')
    test_mask = torch.ones_like(data.test_mask)

    learner = ActiveFactory(args, model, data).get_learner()
...
for num_round in range(args.rand_rounds):
    for num, k in enumerate(args.label_list):
        ...
        res[num_round, num] = active_learn(k, data, model, optimizer, args)
